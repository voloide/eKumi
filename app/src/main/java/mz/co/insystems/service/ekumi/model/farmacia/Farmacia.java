...
public class Farmacia extends AbstractBaseVO {
    ...
    public boolean isAberto(){
        if (sempreAberto == 1) return true;

        java.util.Date currentTime = Calendar.getInstance().getTime();

        if (currentTime.after(horaInicio) && currentTime.before(horaFim)) return true;
    ...
    public String getHorario(){
        if (sempreAberto == 1) return "Aberto 24h";

        String horario = "Aberto das "+horaInicio.getHours()+" - "+horaFim.getHours();
        return horario;
    }

    @JsonIgnore
    public boolean is24h(){
        return sempreAberto == 1;
    }
